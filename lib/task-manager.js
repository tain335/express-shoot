module.exports = (function() {
	"use strict";

	var wm = require('./worker-manager'),
		logger = require('./logger').getLogger('normal'),
		emitter = new (require('events').EventEmitter)(),
		config = require('../config'),
		cache = require('./' + config.taskcache + '-cache'),
		imagePath = config.server.imagePath,
		later = require('later'),
		pendingQ = [],
		ackQ = [],
		boxes = {};
		

	function bindEvent(conn) {
		conn.addListener('close', function() {
			var box = boxes[conn.id],
				ids = Object.keys(box);
			ids.forEach(function(id) {
				pendingQ.push(box[id]);
			});
			delete boxes[conn.id];
		});
		conn.addListener('message', function(message) {
			message = JSON.parse(message);
			switch (message.method) {
				case 'PULL':
					sendTasks(conn);
					break;
				case 'DATA':
					var result = message.data;
						//task = pool[result.uid];
					cache.get(result.uid, function(err, task) {
						if(err) {
							logger.warn('cannot find task: %s from cache!', result.uid);
							return;
						}
						if(task) {
							task.status = result.status;
							task.src = imagePath + task.uid + '.' + task.format;
							//ackQ.push(pool[task.uid]);
							ackQ.push(task);
							emitter.emit('R-' + task.uid, task);
							delete boxes[conn.id][task.uid];
							cache.set(task.uid, task);
						}
					});
					break;
				case 'LOG':
					logger.debug('connection %s log: %s', conn.id, message.data);
					break;
			}
		});
	}

	function sendTasks(conn) {
		var box = boxes[conn.id], tasks;
		if(!pendingQ.length) {
			return;
		}
		if(pendingQ.length < 5) {
			tasks = pendingQ.splice(0);
		} else {
			taks = pendingQ.splice(pendingQ.length - 5, 5);
		}
		conn.send(JSON.stringify({method: 'PUSH', data: tasks}));
		tasks.forEach(function(task) {
			box[task.uid] = task;
		});
	}

	function clearTasks() {
		logger.debug('start clear finished tasks...');
		ackQ.forEach(function(task) {
			//delete pool[task.uid];
			cache.del(task.uid);
		});
		ackQ = [];
	}

	function add(task, cb) {
		//pool[task.uid] = task;
		cache.set(task.uid, task, function(err) {
			if(err) {
				cb(err)
			} else {
				pendingQ.push(task);
				cb(null);
			}
		});
		
	}

	function fetch(uid, cb) {
		cache.get(uid, cb);
	}

	function listen(task, handle) {
		emitter.on('R-' + task.uid, handle);
	}

	function unlisten(task, handle) {
		if(handle) {
			emitter.removeListener('R-' + task.uid, handle);
		} else {
			emitter.removeAllListeners('R-' + task.uid);
		}
	}

	function init(server) {
		logger.debug('init task manager.....');
		cache.init(function(err) {
			if(err) logger.error('cache init happen a error: %s', err);
		});
		server.addListener('connection', function(conn) {
			emitter.emit('conn', conn)
			logger.debug('new connection: %s', conn.id);
			boxes[conn.id] = {};
			bindEvent(conn);
			conn.addListener('close', function(){
				emitter.emit('close', conn);
				logger.error('close connection: %s', conn.id);
			});
		});
		later.setInterval(clearTasks, later.parse.recur().every(3).minute());
	}

	return {
		init: init,
		add: add,
		fetch: fetch,
		listen: listen,
		unlisten: unlisten,
		on: emitter.on.bind(emitter),
		off: emitter.removeListener.bind(emitter)
	}
})();